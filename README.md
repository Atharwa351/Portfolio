# 📊 Atharva Malawade — Revenue & GTM Analytics Portfolio

## TL;DR
I build and own production-grade analytics and GTM data systems under real-world constraints—typically in environments where usable data infrastructure does not exist.

This portfolio demonstrates how fragmented, decaying business data is transformed into **decision-ready operating systems** that directly shape targeting, prioritization, forecasting, and execution.

This is applied, production work—not academic or demo-only analytics.

**Core stack:** SQL · Python · R · Power BI · Tableau  
**Operating model:** Ambiguous business problem → structured analysis → operational decision

---

## Executive Snapshot
- Built and owned the company’s first GTM data platform end-to-end  
- Processed **120M+ rows / ~100GB+ raw CSV data** (Apollo + LinkedIn Navigator)  
- Identified that only **~25% of legacy data was viable**, directly reshaping:
  - ICP definitions  
  - Targeting rules  
  - Outbound prioritization  
- Eliminated CSV-based workflows in favor of query-driven GTM execution  
- Sole owner: architecture, ETL, validation, documentation, and handoff  
- Led interns to build a controlled frontend access layer for non-technical teams  

---

## 🔑 Flagship Project — GTM Data Platform (Production)

📁 **Project folder:**  
👉 `work-projects`

### Problem
Marketing and Lead Generation teams operated on fragmented, aging CSV dumps with:
- No database
- No validation
- No reusable structure
- No safe self-serve access

Silent data decay led to wasted outbound effort and poor targeting decisions.

---

### Solution
Designed and implemented an end-to-end GTM data platform from scratch.

#### Architecture
**Ingest → Validate → Transform → Load → Query**

- Python-based ETL with strict validation logic  
- PostgreSQL schema normalized for inconsistent third-party data  
- Batch ingestion capped at 1,000 rows to ensure reliability under weak infrastructure  
- Explicit trade-off: **data integrity and trust over ingestion speed**

#### Scale
- ~60M rows — LinkedIn Navigator  
- ~76M rows — Apollo.io  
- ~100GB+ raw CSV data  

#### Impact
- Converted idle CSV dumps into a reusable GTM data foundation  
- Eliminated manual CSV-driven workflows  
- Reduced outbound waste caused by stale data  
- Enabled query-driven, intent-led execution  
- Established a repeatable GTM data operating system  

*(schema sanitized.)*

#### Enablement
- Led interns to build a controlled frontend access layer  
- Enabled Sales & Marketing teams to self-serve without direct SQL access  
- Preserved schema integrity while reducing operational risk  

---

## 📈 Revenue & Operations Analytics (Applied)

📁 **Project folder:**  
👉 `applied-analytics`

### Revenue Forecasting
- Weekly revenue forecasting using **ARIMA** and **Exponential Smoothing**
- Designed for pipeline planning and operational predictability—not model novelty

### CLTV & Expense Forecasting
- Identified top **20% of accounts contributing ~65% of projected CLTV**
- Used for CAC optimization, prioritization, and planning decisions

---

## 📊 Dashboards (Decision-Focused)

📁 **Project folder:**  
👉 `dashboards`

Dashboards are designed to surface **decision signals**, not visual polish.

### CRM Account Segmentation (Tableau)
- Subsidiaries ≈ **18% of total revenue**
- Used for ABM focus and account prioritization

### Stock Volatility & Trend Analysis (Power BI)
- Trend shifts and anomaly detection  
- Used for scenario planning and timing analysis  

---

## 🧠 Market & Customer Intelligence

📁 **Project folder:**  
👉 `applied-analytics`

Analytics used to inform GTM trade-offs, not academic insight.

### Conjoint Analysis
- Price and RAM explained **>70% of preference utility**
- Used to evaluate GTM trade-offs

### Market Basket Analysis (R)
- Cross-sell and bundling insights

---

## 📚 Strategy & Research (Supporting)

📁 **Project folder:**  
👉 `strategy-and-research`

Included to demonstrate business framing and structured reasoning:
- GTM Strategy — Birla Paints (SWOT + Porter)
- Urban GTM Impact Study — Street Vendors

---

## 🧪 Models & Simulations (Exploratory)

📁 **Project folder:**  
👉 `models-and-simulations`

Exploratory models used to build analytical intuition—not deployed production ML:
- Salary Prediction  
- Lead Scoring  
- Manual Gradient Descent  
- MNIST Classification  
- Revenue Simulation via Neural Networks  

---

## How I Work
- Translate ambiguous business problems into structured analytical questions  
- Build systems designed for **use**, not admiration  
- Optimize for clarity, reliability, and adoption  
- Work cross-functionally with Sales, Marketing, and Operations  
- **Sales background → analytics optimized for actionable signals, not theoretical perfection**

---

## About Me
MBA — Data Analytics  
Experience across Sales, Revenue Operations, and Analytics  

**Core strength:** turning messy business reality into structured, executable systems.

---

## Let’s Connect
If you’re building data-driven operating systems in revenue or GTM environments—and value execution over theory—I’m open to conversations.
